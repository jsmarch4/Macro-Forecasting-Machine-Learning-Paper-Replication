...
# Use the two informative diagnostic cases only.
MODEL_CASES = [
    {
        "model": "median_linear",
        "tau": 0.50,
...
def prepare_validation_cache():
    """
    Precompute expanding-window validation datasets for 1980-1999 only.
    """
...
def train_one_window(
    ...
    """
    Train one expanding-window model using plain SGD.
    """
...
def recursive_validation_forecasts(
    ...
    """
    Produce recursive validation forecasts for one model case and
    one learning rate. No observations after 1999-12 are used.
    """
    ...
    for i, item in enumerate(validation_cache):
        ...
        if i % 50 == 0:
            print(
                f"{case['model']} | lr={learning_rate:g} | "
...
def main():
    ...
    # Rank learning rates separately within each diagnostic model.
    ...
    output_file = (
        DEBUG_DIR
        / "learning_rate_validation_comparison.csv"
    )
