...
def recursive_validation_forecasts(
    ...
    for i, item in enumerate(validation_cache):
        ...
        if i % 50 == 0:
            print(
                f"{case['model']} | momentum={momentum:g} | "
...
def main():
    ...
    for case in MODEL_CASES:
        for momentum in MOMENTUM_VALUES:
            ...
            print(
                ...
                f"tau={case['tau']} | "
                f"lr={LEARNING_RATE} | "
                f"epochs={case['epochs_initial']}/"
                f"{case['epochs_update']} | "
                f"momentum={momentum}"
            )
            print("=" * 80)
            ...
            summary_rows.append({
                ...
                "momentum": momentum,
                "momentum_reset_each_window": True,
    ...
    output_file = (
        DEBUG_DIR
        / "momentum_validation_comparison.csv"
    )
    summary.to_csv(output_file, index=False)

    print("\nValidation-only momentum comparison")
