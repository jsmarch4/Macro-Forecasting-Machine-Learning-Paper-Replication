...
ARCHITECTURE_GRID = [
    {"nonlinear_layers": layers, "hidden_dim": dim, "alpha": alpha}
    for layers in [1, 2]
    for dim in [2, 4, 8]
...
print("\n" + "=" * 80)
print(f"Running DNN search for tau={tau:.2f}")
...
if not np.isfinite(test_loss):
    retry_learning_rate = 0.0001
    print(
        "Non-finite test loss; retrying selected model with "
        f"learning rate {retry_learning_rate}."
    )
    ...
    test_loss = float(
        average_pinball_loss(test_forecasts, tau)
    )

if not np.isfinite(test_loss):
    raise RuntimeError(
        f"Non-finite test loss after retry for tau={tau:.2f}."
    )
